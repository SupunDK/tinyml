...
parser.add_argument('--mode', type=str, default='total', choices=['total','train','eval','test', 'equal_dist'], help="evaluation mode.")
# classifier hyperparams
parser.add_argument('--subset', type=int, default=1250)
parser.add_argument('--factor', type=float, default=2.075)
parser.add_argument('--thresh', type=float, default=9.211)
...
def main():
    ...
    file = open("data_indices/"+args.mode+"_indice.csv")
    total_names = set(line.split(',')[1][:3] for line in file.readlines()[1:])
    total_fb = 0
    good_count = 0
    total_time = 0
    for name in total_names:
        ...
        dataloader = DataLoader(dataset, batch_size=args.batch_size, shuffle=False, num_workers=0)
        start = time.time()
        fb = peak_based_eval(dataloader, factor, method, threshold, subset, tqdm_)
        duration = (time.time() - start) / len(dataset)
        print("Time taken per signal:", duration)

        total_fb += fb
        total_time += duration
        if fb >= 0.95: good_count += 1
    avg_fb = total_fb / len(total_names)
    gen_score = good_count / len(total_names)
    avg_time = total_time / len(total_names)
    print("Fbeta:", avg_fb, "Generalization score:", gen_score, "Average time:", avg_time)
...
def countPeaks(chunk,factor,method,inp_len=1250):
    ...
    for i in range(2, len(chunk), 2):
        ...
        std += chunk[i] * chunk[i]
    mean /= len(chunk)
    std = np.sqrt(std/len(chunk) - mean * mean)
...
def peak_based_eval(dataloader, factor, method, threshold_grid, subset=1250, tqdm_=False):
    ...
    for iter, data_val in enumerate(tqdm(dataloader) if tqdm_ else dataloader):
        ...
        if args.verbose:
            print(f'pred: {pred}, label: {label}', 'peaks sampled: ', numPeaks)
